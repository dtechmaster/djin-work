...
1. Create **Jira Task** with everything documented **or** reuse existing Tasks if applicable.
2. Reassign to the Responsible party (usually move back one column in Jira).
   * If the Responsible party is out of scope, **escalate to Management**.
   * Ex: task exists, is in column D ("Operation Tests"). Comment and move back to column C ("To Do").
   Note: letters are used to explain columns because actual column names may change over time/project. But the logic is clear and applicable to any project.
3. Record in Slack / Task comment that:
   * the checklist was executed → **[→ Access Checklist](../../../toolbox/QA-ST-0001/checklist-en.md)**
   * problems were found
